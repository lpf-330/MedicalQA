...
class EntityRelationsCollectionCreator:
    """实体关系向量集合创建器"""
    ...
    def create_collection_schema(self):
        ...
        try:
            fields = [
                ...
                FieldSchema(
                    name="neo4j_relation_id",
                    dtype=DataType.VARCHAR,
                    max_length=128,
                    description="Neo4j关系elementId"
                )
            ]
            
            schema = CollectionSchema(
                fields=fields,
                description="实体关系向量集合，用于存储医疗实体关系的向量表示",
            ...
            self.logger.info(f"  - relation_type: VARCHAR(50)")
            self.logger.info(f"  - source_entity_name: VARCHAR(255)")
            self.logger.info(f"  - source_entity_type: VARCHAR(50)")
            self.logger.info(f"  - target_entity_name: VARCHAR(255)")
            self.logger.info(f"  - target_entity_type: VARCHAR(50)")
            self.logger.info(f"  - relation_description: VARCHAR(500)")
            self.logger.info(f"  - neo4j_relation_id: VARCHAR(128)")
    ...
    def run(self):
        """执行完整的集合创建流程"""
        self.logger.info("=" * 60)
        self.logger.info("开始创建 EntityRelations 向量数据库集合")
        self.logger.info("=" * 60)
...
def main():
    """主函数"""
    creator = EntityRelationsCollectionCreator()
    
    try:
        creator.run()
        print("\n✓ 集合创建成功！")
        print(f"✓ 集合名称: entity_relations")
        print(f"✓ 向量维度: 1024")
