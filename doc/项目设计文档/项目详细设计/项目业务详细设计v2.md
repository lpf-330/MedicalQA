...
| 医学分析模型  | 健康风险因子分类模型：iic/nlp_raner_named-entity-recognition_chinese-base-cmeee<br />健康风险预测模型：MedGemma-1.5-4B-IT | 9G       | 2个轻量化医学模型总和                                        |
...
stateDiagram-v2
    ...
    state KNOWLEDGE_RETRIEVAL {
        [*] --> 并行检索
        note right of 并行检索 : 内部并发执行“Neo4j图谱检索”和“ 向量增强检索”
        并行检索 --> [*]
    }
...
#### 2.3 KNOWLEDGE\_RETRIEVAL状态的并发实现（双路并行：Neo4j检索+向量增强检索）

该状态是健康咨询的核心技术亮点——**双路并行检索**：（以下是代码示例）
...
#### 2.4 健康咨询性能优化与降级策略（并发控制/超时/降级矩阵）
...
#### 3.3.1 PARALLEL\_PROCESSING状态的并发控制实现（三路并行）

该状态是健康报告生成的核心技术亮点——**三路并行**：（以下是代码示例）
...
#### 3.3.2 健康报告性能优化与降级策略（并发/超时/降级矩阵）

##### 并发控制策略

| 阶段                 | 并行度   | 实现方式                             |
| -------------------- | -------- | ------------------------------------ |
| PARALLEL\_PROCESSING | 10路并行 | asyncio.gather(8维度+Neo4j+向量检索) |
...
| MULTI\_ANALYSIS      | 30秒     |                       |
| PARALLEL\_PROCESSING | 60秒     |                       |
| REPORT\_GENERATION   | 240秒    |                       |
