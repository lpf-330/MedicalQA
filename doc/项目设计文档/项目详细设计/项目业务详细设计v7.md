...
4. **显存重分配**: Qwen3-4B-AWQ(~7.7G,4并发) + bge(~2G) + Apollo(~1G) + nlp_raner(~1G) + MedPsy-4B-AWQ(~6G,3并发) = ~17.7GB / 22.5GB，剩余~4.8GB
...
5. **5模型共存显存重分配**: Qwen3-4B-AWQ(~7.7G,4并发) + bge(~2G) + Apollo(~1G) + nlp_raner(~1G) + MedPsy-4B-AWQ(~6G,3并发) = ~17.7GB / 22.5GB，剩余~4.8GB
...
5. **5模型共存显存分配**: Qwen3-4B-AWQ(~7.7G,4并发) + bge(~2G) + Apollo(~1G) + nlp_raner(~1G) + MedPsy-4B-AWQ(~6G,3并发) = ~17.7GB / 22.5GB，剩余~4.8GB
...
| 8 | **知识精炼** | ComprehensiveHealthAnalysisAgent | LLM精炼整合知识 | 去冗余、去重复、去不相关，输出JSON格式（设计目标：定义精炼规则和结构化输出格式） |
| 9 | **充分性判断（基于加权平均的充分性评分）** | ComprehensiveHealthAnalysisAgent | 评估维度知识是否充分(v5.14：从EvaluateRelevance结果提取dimension_sufficiency，不再独立调用LLM) | EvaluateRelevance的dimension_sufficiency字段+规则引擎降级，阈值0.6 |
...
| **PlanRetrieval** | LLM规划检索策略（新增） | 检索路径规划结果 |
| **ParallelRetrieve** | 按检索策略并行检索 | 详见内部子状态 |
| **KnowledgeSelection** | LLM筛选高质量知识（新增） | 筛选后的知识列表 |
| **Deduplicate** | 去重检索结果 | 基于实体ID去重,合并重复结果。同时维护知识黑名单（excluded_entity_ids），补充检索时排除不相关实体 |
| **HybridRelevance** | 混合相关性评分（LLM评估+向量语义） | 维度相关性评分+用户相关性评分 |
| **EvaluateSufficiency** | 充分性评估（从ParallelRetrieve子状态提升为独立状态） | {"sufficiency_score": 0.75, "gaps": [...]} |
| **PatchPathPlan** | 补充检索路径规划（新增） | 补充检索路径 |
...
相比v6的6状态设计，新增5个LLM决策状态（PlanRetrieval、KnowledgeSelection、HybridRelevance、EvaluateSufficiency、PatchPathPlan），提供更精细的检索控制能力。

**ParallelRetrieve内部子状态**:

| 子状态 | 说明 | 输出示例 |
|---------|------|--------|
| **ExecuteRetrieval** | 按PlanRetrieval规划的检索路径执行检索 | raw_knowledge |

InitDimensions已合并到BuildQueries父状态中，CrossDimensionShare在当前实现中未被调用。LLMJudgeSufficiency已提升为独立的EvaluateSufficiency状态。SupplementaryRetrieve由PatchPathPlan→ParallelRetrieve循环替代。
...
stateDiagram-v2
    ...
    PlanRetrieval --> ParallelRetrieve
    ...
    ParallelRetrieve --> KnowledgeSelection
    KnowledgeSelection --> Deduplicate
    Deduplicate --> HybridRelevance
    HybridRelevance --> EvaluateSufficiency
    
    EvaluateSufficiency --> RefineKnowledge: 充分(>=0.6)
    EvaluateSufficiency --> PatchPathPlan: 不充分(<0.6)
    
    PatchPathPlan --> ParallelRetrieve: 补充检索
...
1. **向量候选筛选(Vector Candidate Screening)**: 使用VectorRetrievalTool的hybrid_search方法，对每个维度的查询文本进行三集合混合检索，获取候选实体名称列表
