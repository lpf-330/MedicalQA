...
**v6.2更新**:
1. **健康评估模型升级**: MedPsy-1.7B → MedPsy-4B(Qwen3-4B-Thinking-2507架构，FP16加载，Thinking模型)，总显存~8.5GB
2. **Thinking模型适配**: MedPsy-4B基于Qwen3-4B-Thinking-2507，思维链已融入权重不可禁用；通过--reasoning-parser qwen3分离thinking到reasoning_content字段，content输出纯净JSON
...
5. **Qwen3-4B-AWQ并发调整**: --max-running-requests 2→1，--max-total-tokens 16384→8192，释放~2.3GB KV Cache为MedPsy-4B提供显存
6. **5模型共存显存重分配**: Qwen3-4B-AWQ(~7.7G,1并发) + bge(~2G) + Apollo(~1G) + nlp_raner(~1G) + MedPsy-4B(~8.5G) = ~20.2GB / 22.5GB，剩余~2.3GB

**v6.0更新**:
1. **推理框架迁移**: 从vLLM迁移到SGLang，双SGLang实例部署（Qwen3-4B-AWQ :30000主推理 + MedPsy-4B :30001健康评估）
2. **大语言模型更换**: Qwen3-4B-Instruct-2507(FP16) → Qwen3-4B-AWQ(AWQ 4bit量化)，权重2.57GB + KV Cache 2.26GB(1并发) + CUDA Graph 1.24GB = ~7.7GB
3. **健康评估模型更换**: MedGemma-1.5-4B-IT → MedPsy-4B(Qwen3-4B-Thinking-2507架构，FP16加载，Thinking模型)，总显存~8.5GB，部署在SGLang :30001独立实例
4. **NER模型显存更正**: nlp_raner 2G → 1G（实测仅0.4GB）
5. **5模型共存显存分配**: Qwen3-4B-AWQ(~7.7G,1并发) + bge(~2G) + Apollo(~1G) + nlp_raner(~1G) + MedPsy-4B(~8.5G) = ~20.2GB / 22.5GB，剩余~2.3GB
6. **SGLang引擎配置与稳定性**: 移除Sleep Mode（SGLang无此特性），使用--max-total-tokens控制KV Cache，--mem-fraction-static=0.78，双实例故障隔离
7. **MedPsy评估引擎适配**: 调用方式从sglang_model资源池改为medpsy_model独立资源池（SGLang :30001），Thinking模型通过--reasoning-parser qwen3分离思维链，_extract_content()只返回content字段
...
| ---------- | ------------------------------------------------------------ | --------------- | ------------ | ------------------------------------------- |
| 大语言模型      | Qwen3-4B-AWQ                                                  | 5G              | 咨询+报告        | 4bit量化(AWQ),1并发(--max-running-requests=1)。咨询业务:对话理解和回答生成(流式);报告业务:报告生成(流式)   |
| 向量编码模型     | BAAI/bge-large-zh-v1.5                                       | 2G              | 咨询+报告        | 1024维向量。咨询业务:语义编码和知识匹配;报告业务:8维度知识检索和Agent检索 |
| 意图分类模型     | FreedomIntelligence/Apollo-0.5B                              | 1G              | 咨询           | 咨询业务:识别用户咨询意图(文本分类，不具备NER能力)                         |
| 医学命名实体识别模型 | iic/nlp\_raner\_named-entity-recognition\_chinese-base-cmeee | 1G              | 咨询+报告        | 咨询业务QUERY_PARSE环节:医疗实体提取(识别疾病、症状、药物等实体);报告业务DATA_PARSE环节:风险因子提取(从病史文本中识别疾病、症状实体)和医疗实体提取(从病史文本中识别疾病、症状、药物、检查项目实体) |
| 健康风险预测模型   | MedPsy-4B                                                  | 9G              | 报告           | 报告业务COMPREHENSIVE_HEALTH_ANALYSIS环节:作为评估引擎对健康评估框架的各子指标进行医学推理评估(SGLang :30001独立实例,Thinking模型,--reasoning-parser qwen3),输出子指标评分+理由,最终按算法公式汇总计算健康评分、风险等级、疾病风险评分 |
...
| --context-length | **16384** | 上下文窗口扩大到16K，支持健康报告撰写的大量内容输入 |
| --mem-fraction-static | 0.78 | SGLang静态显存分配比例 |
...
| 配置项 | 配置值 | 说明 |
|-------|-------|------|
| --max-total-tokens | **8192** | v6.2更新：1并发下KV Cache，释放显存为MedPsy-4B部署 |
| --max-running-requests | **1** | v6.2更新：并发数从2降回1，释放~2.3GB显存供MedPsy-4B使用 |
| KV Cache显存 | ~2.26 GiB | v6.2更新：--max-total-tokens=8192下，1并发KV Cache |
...
| KV Cache | 2.26 GiB | --max-total-tokens=8192，1并发 |
...
1. **AWQ量化方案选择原因**：
   ...
   - 在22GB显存约束下，节省的6GB显存正好用于MedPsy-4B独立部署（~8.5GB）

2. **SGLang关键配置说明**：
   - `--context-length 16384`：上下文窗口大小，支持健康报告Prompt的完整输入
   - `--mem-fraction-static 0.78`：静态显存分配比例，预留22%给其他模型和系统
   - `--max-total-tokens 8192`：1并发下KV Cache容量，释放显存供MedPsy-4B使用
   - `--max-running-requests 1`：1并发，Qwen3-4B-AWQ释放显存为MedPsy-4B部署腾出空间
   - `--attention-backend triton`：AWQ在sm_75上Triton回退，非最优但功能可用

3. **并发数调整原因**（v6.2更新）：
   - MedPsy-1.7B升级为MedPsy-4B(Thinking模型)，显存从~4.1GB增加到~8.5GB，需释放额外~4.4GB
   - 将Qwen3-4B-AWQ并发数从2降回1，--max-total-tokens从16384降为8192，释放~2.3GB KV Cache
   - 5模型共存总显存~20.2GB / 22.5GB，剩余~2.3GB，仍可满足运行时需求
   - 1并发下SGLang continuous batching仍可正常调度，报告生成中多步推理改为串行执行
   - 报告生成中相关性评估、知识精炼等多步推理可在不同请求slot中交替执行，减少等待时间

##### MedPsy-4B 健康评估模型配置（v6.2更新）
...
| 加载精度 | FP16 | 4B参数FP16权重~8GB |
| --context-length | **4096** | 健康评估场景上下文较短，4096足够 |
| --max-total-tokens | **4096** | 单次健康评估输入Prompt+输出控制在4096 token内 |
| --mem-fraction-static | 0.50 | v6.2更新：降低静态分配比例，为Thinking模型预留更多运行时显存 |
| --dtype | float16 | 推理数据类型 |
| --attention-backend | triton | sm_75上Triton后端 |
| --disable-cuda-graph | — | v6.2更新：禁用CUDA Graph以节省显存（~1.5GB），Thinking模型CUDA Graph占用量大 |
| --reasoning-parser | **qwen3** | v6.2新增：分离Thinking输出，reasoning_content字段存思维链，content字段输出纯净JSON |
...
| 组件 | 显存占用 | 说明 |
|------|---------|------|
| 模型权重 | ~8.0 GiB | FP16权重，4B参数 |
| KV Cache | ~0.3 GiB | --max-total-tokens=4096 |
| 其他开销 | ~0.2 GiB | 框架开销（已禁用CUDA Graph） |
| **总计** | **~8.5 GiB** | 健康评估实例SGLang总显存占用 |

**配置说明**:

1. **MedPsy-4B选型与升级原因**（v6.2更新）：
   - MedPsy-4B基于Qwen3-4B-Thinking-2507，相比MedPsy-1.7B(1.7B参数)模型能力更强，医学推理评估质量更高
   - 4B参数量FP16需~8GB显存，通过Qwen3-4B-AWQ并发降为1释放~2.3GB显存来容纳
   - Thinking模型的思维链融入权重不可禁用，但通过--reasoning-parser qwen3可将思维链分离到reasoning_content字段，content字段输出纯净JSON

2. **Thinking模型适配**（v6.2新增）：
   - MedPsy-4B基于Qwen3-4B-Thinking-2507，思维链已融入模型权重，无法通过enable_thinking=False完全禁用
...
3. **max_total_tokens=4096限制**：
   - MedPsy用于单次健康评估，输入Prompt+输出评估结果控制在4096 token内
   - 需优化评估Prompt精简度，确保输入+输出不超过4096 token
   - 超出4096的请求将被截断，需在业务逻辑中控制Prompt长度
...
| 健康评估实例 | :30001 | MedPsy-4B | COMPREHENSIVE_HEALTH_ANALYSIS环节:子指标评估+风险因子评估 | ~8.5GB |

**双实例故障隔离机制**: 两个SGLang实例独立部署在不同端口，任一实例崩溃不影响另一实例。主推理实例(Qwen3-4B-AWQ)崩溃时仅影响咨询回答生成(流式)、报告生成(流式)和相关性评估；健康评估实例(MedPsy-4B)崩溃时仅触发降级规则引擎评估。

**SGLang稳定性保障(v6.1更新)**:

| 保障措施 | 说明 |
|---------|------|
| --max-total-tokens精确控制 | Qwen3-4B-AWQ通过--max-total-tokens=8192精确控制KV Cache大小，1并发；MedPsy-4B通过--max-total-tokens=4096控制评估token预算 |
| --mem-fraction-static | Qwen3-4B-AWQ: 0.78；MedPsy-4B: 0.50（降低静态分配，为Thinking模型预留运行时显存） |
| SGLang无Sleep Mode | SGLang不使用Sleep Mode，通过--max-total-tokens和--max-running-requests保障稳定性 |
| 批量推理 | 8维度评估合并为一次generate调用，减少引擎运行次数 |
| 双实例故障隔离 | Qwen3-4B-AWQ和MedPsy-4B独立实例，互不影响 |
| --max-running-requests=1 | 1并发，释放显存为MedPsy-4B部署腾出空间 |
| --reasoning-parser qwen3 | MedPsy-4B Thinking模型适配，分离思维链到reasoning_content，content输出纯净JSON |
| --disable-cuda-graph | MedPsy-4B禁用CUDA Graph节省~1.5GB显存 |
...
| 健康评估模型 | 复用Qwen3-4B | MedPsy-4B独立实例(:30001) |
| 模型权重显存 | ~7.61 GiB (FP16) | 2.57 GiB (AWQ) |
| KV Cache控制 | gpu_memory_utilization=0.78 | --max-total-tokens=8192 (精确,1并发) |
| Sleep Mode | 启用但不可用(EngineCore崩溃) | 无Sleep Mode特性 |
| 引擎实例数 | 1个(3模型服务共享) | 2个(主推理:30000 + 健康评估:30001) |
| 健康评估方式 | 复用Qwen3-4B + 医学Prompt | MedPsy-4B独立评估(Thinking模型,--reasoning-parser qwen3) |
| 故障隔离 | 无(单实例崩溃影响全部) | 有(双实例独立) |
| 并发控制 | max_num_seqs=1 | --max-running-requests=1 |
...
**处理逻辑**:
1. NLP实体抽取(使用nlp_raner模型识别疾病、症状、药物等医疗实体)
2. 意图识别(使用Apollo-0.5B模型进行文本分类)
3. 查询改写(基于会话历史优化查询)
...
| 12 | **SGLang引擎稳定性保障** | ComprehensiveHealthAnalysisAgent+SGLangAdapterImpl | 双SGLang实例故障隔离 + --max-total-tokens控制KV Cache + --max-running-requests=1(v6.2更新,释放显存供MedPsy-4B) | 批量推理+降级策略+推理超时保护(v5.15更新)；双实例故障隔离(v6.0新增) |
...
| COMPREHENSIVE\_HEALTH\_ANALYSIS          | 180秒  | Agent失败使用已完成结果继续,MedPsy降级为规则计算  |
...
| 步骤 | 使用模型 | 降级方案 |
|------|----------|----------|
| 步骤1:异常指标提取 | 无(规则引擎，输入是数值不需要NER) | / |
| 步骤2:风险因子提取 | **nlp_raner**(通过NerModelTool识别病史文本中的疾病、症状实体) | 规则引擎(关键词匹配) |
| 步骤3:医疗实体提取 | **nlp_raner**(通过NerModelTool识别疾病、症状、药物、检查项目实体) | 规则引擎(关键词匹配) |
| 步骤4:特殊规则应用 | 无(规则引擎) | / |
| 步骤5:生成分析摘要 | 无(字符串拼接) | / |
...
# 一次批量推理（SGLang内部continuous batching并行调度）
...
**批量推理优势**：
- SGLang内部使用continuous batching，8个prompt共享forward pass
...
- health_assessment: 健康评估结果
  ...
  - reasoning: 医学推理过程（reasoning字段由MedPsy-4B子指标评估的sub_indicator_scores中各reason字段拼接汇总生成，非独立LLM输出字段）
...
| 降级策略 | 批量推理失败→串行推理→规则引擎默认评分（详见3.3.7降级策略） |
| --context-length=16384 | SGLang保持16K上下文窗口，支持8维度批量评估的长prompt输入 |
| 双实例故障隔离 | Qwen3-4B-AWQ(:30000)和MedPsy-4B(:30001)独立实例，健康评估失败不影响报告生成 |
...
- 5维度健康评估（MedPsy-4B :30001独立实例）
- 6风险因子评估（MedPsy-4B :30001独立实例）
- 健康咨询业务的意图分类（Apollo-0.5B）和实体提取（nlp_raner）
- 健康咨询业务的知识检索Agent决策（Qwen3-4B-AWQ :30000）

**重要约束(v6.0更新)**：LLM推理防重复设计中的`max_new_tokens=512`仅限制单次推理的输出token数，**不得修改`--context-length=16384`**。`--context-length`是SGLang引擎的全局上下文窗口大小，健康报告生成业务的Prompt输入（包含用户档案、8维度知识检索结果、健康评估结果等）需要16K上下文窗口才能完整容纳。若减小`--context-length`，将导致报告生成Prompt被截断，业务失败。同时，MedPsy-4B的`--max-total-tokens=4096`限制健康评估单次调用的总token数（输入+输出），评估Prompt需精简以避免截断。
...
- **MedPsy-4B引擎(SGLang :30001)**: 对每个子指标进行医学推理评估，输出子指标评分(0-1归一化)+理由
- **最终计算**: 按算法公式汇总各子指标评分，输出健康评分(0-100)、风险等级(低/轻/中/高)、疾病风险评分列表
- **降级方案**: MedPsy-4B不可用时，使用3.4节规则引擎降级评估
...
2. **健康评估失败降级**:
   - MedPsy-4B模型不可用:降级为规则计算(详见3.4节降级算法实现)
...
> 本章的所有算法服务于**健康报告生成**业务,作为MedPsy-4B评估引擎的降级方案。**当MedPsy-4B模型不可用时,使用这些规则算法对子指标进行评估。**主路径设计详见3.3.6节HealthAssessmentChain详细设计。
...
**设计理念(v6.0更新)**: 采用"评估框架+LLM评估引擎"的分层设计，评估框架定义评估维度、子指标、权重和计算公式，MedPsy-4B作为评估引擎对每个子指标进行医学推理评估，最终按算法公式汇总计算。MedPsy-4B部署在SGLang :30001独立实例，通过HTTP API调用，单次输入+输出限制4096 token。
...
flowchart TD
    ...
    subgraph 推理层["MedPsy-4B评估引擎（SGLang :30001，并行执行）"]
        B1["维度1:生理指标评估"]
        B2["维度2:生活方式评估"]
        B3["维度3:病史风险评估"]
        B4["维度4:心理状态评估"]
...
**MedPsy-4B子指标评估Prompt模板(v5.15更新-精简版，v6.0适配4096 token限制)**:
...
**MedPsy-4B风险因子评估Prompt模板(v5.15更新-精简版，v6.0适配4096 token限制)**:
...
**MedPsy-4B模型调用的通用约束(v6.0更新)**:

1. **上下文长度限制**: SGLang --max-total-tokens=4096，单次调用输入+输出总token数不超过4096
...
| 版本       | 日期             | 更新内容                                                                                            |
| -------- | -------------- | ----------------------------------------------------------------------------------------------- |
| **v6.2** | **2026-06-03** | **1. 健康评估模型升级:MedPsy-1.7B→MedPsy-4B(Qwen3-4B-Thinking-2507,4B参数,FP16~8.5GB);2. Thinking模型适配:--reasoning-parser qwen3分离思维链,--disable-cuda-graph节省显存,--mem-fraction-static=0.50;3. Prompt优化:系统提示词改为时间+字数双重约束最小化思考输出;4. max_tokens调整:medpsy_max_tokens 1024→2048,medpsy_batch_max_tokens 512→2048;5. Qwen3-4B-AWQ并发调整:max-running-requests 2→1,max-total-tokens 16384→8192,释放~2.3GB显存;6. 5模型共存显存重分配:~20.2GB/22.5GB,剩余~2.3GB;7. _extract_content()适配:只返回content字段,丢弃reasoning_content** |
...
| **v6.0** | **2026-06-01** | **1. 推理框架迁移:vLLM→SGLang,双SGLang实例(Qwen3-4B-AWQ :30000 + MedPsy-4B :30001);2. 大语言模型更换:Qwen3-4B-Instruct-2507(FP16)→Qwen3-4B-AWQ(AWQ 4bit),显存7G→3G;3. 健康评估模型更换:MedGemma-1.5-4B-IT(7G)→MedPsy-4B(Qwen3ForCausalLM,FP16,显存4G,max_total_tokens=4096),显存7G→4G;4. NER模型显存更正:2G→1G;5. 5模型共存显存:~15.4GB/22.5GB,剩余~7.1GB;6. SGLang引擎配置重写:--max-total-tokens控制KV Cache,--mem-fraction-static=0.78,移除Sleep Mode,双实例故障隔离;7. MedPsy评估引擎适配:独立medpsy_model资源池(SGLang :30001),Prompt精简适配4096 token限制;8. 附录E重写:SGLang双实例资源管理;9. 除业务功能最终的返回输出外,所有模型调用使用非流式** |
...
| medpsy_max_tokens | 2048 | HealthAssessmentConfig | v6.2更新：MedPsy-4B(Thinking模型)调用默认最大生成token数 |
| medpsy_batch_max_tokens | 2048 | HealthAssessmentConfig | v6.2更新：MedPsy-4B(Thinking模型)批量调用最大生成token数 |
...
| 模型 | 上下文窗口 | 显存占用 | 用途 |
|------|-----------|---------|------|
| Qwen3-4B-AWQ | 16384 tokens (--context-length) | ~7.7 GiB (AWQ 4bit,1并发) | 咨询回答生成(流式)、报告生成(流式)、相关性评估、知识精炼(SGLang :30000) |
| MedPsy-4B | 4096 tokens (--max-total-tokens) | ~8.5 GiB (FP16,Thinking模型) | 健康评估引擎:子指标评估+风险因子评估(SGLang :30001独立实例,--reasoning-parser qwen3) |
| Apollo-0.5B | ~32K tokens (原生) | ~1G | 意图分类和实体提取 |
| BAAI/bge-large-zh-v1.5 | 512 tokens (max_length) | ~2G | 向量编码 |

> **实测数据** (2026-06-03, RTX 2080 Ti 22.5GB, v6.2 SGLang双实例1并发+MedPsy-4B):
> - Qwen3-4B-AWQ SGLang实例(:30000): 权重2.57GB + KV Cache 2.26GB(1并发) + CUDA Graph 1.24GB + 其他 ~1.63GB = ~7.7GB
> - MedPsy-4B SGLang实例(:30001): 权重~8.0GB (FP16,4B参数) + KV Cache + 其他 = ~8.5GB (禁用CUDA Graph)
> - bge + Apollo + nlp_raner: ~2G + ~1G + ~1G = ~4GB
> - 5模型共存总显存: ~20.2GB / 22.5GB (10.2%剩余, ~2.3GB)
...
|-----------|---------|-----------|------|------|------|
| sglang_model | sglang_model | :30000 | Qwen3-4B-AWQ | ~7.7GB | 咨询回答生成(流式)、报告生成(流式)、相关性评估、知识精炼 |
| medpsy_model | medpsy_model | :30001 | MedPsy-4B | ~8.5GB | COMPREHENSIVE_HEALTH_ANALYSIS环节:子指标评估+风险因子评估(Thinking模型,--reasoning-parser qwen3) |

| 模型服务 | 文件 | 资源获取方式 | 使用的SGLang实例 |
|---------|------|-------------|-----------------|
| ConsultModelService | `consult_model_service.py` | `acquire("sglang_model")` | :30000 (Qwen3-4B-AWQ) |
| ReportModelService | `report_model_service.py` | `acquire("sglang_model")` | :30000 (Qwen3-4B-AWQ) |
| MedPsyModelService | `medpsy_model_service.py` | `acquire("medpsy_model")` | :30001 (MedPsy-4B) |

> **设计说明(v6.2)**: MedPsy-4B现在拥有独立的medpsy_model资源池，部署在SGLang :30001独立实例上。与v5.x的"复用Qwen3-4B + 医学Prompt"方案相比，v6.0实现了真正的评估引擎独立部署，具备以下优势：(1)故障隔离——MedPsy实例崩溃不影响Qwen3-4B的报告生成和咨询回答；(2)独立调优——可根据健康评估任务特点独立调整采样参数；(3)显存可控——MedPsy-4B占~8.5GB，独立管理更精确。v6.2升级为Thinking模型，通过--reasoning-parser qwen3分离思维链，系统提示词优化为时间+字数双重约束最小化思考输出。
...
| MedPsy-4B (:30001) 崩溃 | 健康综合评分、疾病风险评分、风险等级判定 | 降级为规则引擎评估(详见3.4节降级算法实现) |
...
| medpsy_max_tokens | 2048 | HealthAssessmentConfig | v6.2更新：MedPsy-4B(Thinking模型)单次调用默认最大生成token数，思维链额外消耗token |
| medpsy_batch_max_tokens | 2048 | HealthAssessmentConfig | v6.2更新：MedPsy-4B(Thinking模型)批量调用最大生成token数，思维链额外消耗token |

> **v6.2说明**: MedPsy-4B为Thinking模型，思维链消耗额外token，max_tokens=1024会导致风险因子评估场景content被截断为空。Qwen3-4B-AWQ的--max-total-tokens=8192提供1并发KV Cache空间。MedPsy-4B的--max-total-tokens=4096为单次健康评估调用的总token预算（输入+输出），评估Prompt需精简以适应此限制。
...
#### F.2 HealthAssessmentChain MedPsy评估解析失败

**问题**: 5维度评估中D1(生理指标)、D2(生活方式)、D4(心理状态)的MedPsy批量推理结果JSON解析失败，降级为规则引擎。

**分析(v6.0注: MedPsy-4B的max_total_tokens=4096,评估Prompt需精简)**:
...
- **v6.0改进方向**: MedPsy-4B独立实例部署后，评估Prompt需控制在3000字符以内（适配4096 token限制），可能减少截断问题
