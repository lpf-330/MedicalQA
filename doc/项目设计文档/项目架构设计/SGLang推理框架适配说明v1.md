...
**生产场景（双实例 — Qwen3-4B-AWQ + MedPsy-1.7B）**：

```bash
#!/bin/bash
# 实例 1：主推理模型 Qwen3-4B-AWQ
/home/ai_env/miniforge3/envs/SGLangTest/bin/python -m sglang.launch_server \
...
# 实例 2：健康评估模型 MedPsy-1.7B
/home/ai_env/miniforge3/envs/SGLangTest/bin/python -m sglang.launch_server \
  --model-path /home/project/MedicalQA/base_models/MedPsy-1.7B \
  --context-length 4096 \
...
13. **`--reasoning-parser qwen3` 输出行为**：启用后思考内容输出到 `reasoning_content` 字段，`content` 为空。客户端需检查 `reasoning_content` 字段
...
项目原计划使用 Qwen3-4B FP16 推理，模型权重约 7.7 GB。在 22 GB 显存上与 MedPsy-1.7B + 3 个小模型共存时，显存极度紧张（5 模型共存需 ~17 GB 仅权重）。
...
bge-large-zh、Apollo-0.5B、nlp_raner 为非生成式模型，使用 transformers/sentence-transformers 直接加载，不经过 SGLang：

| 模型 | 加载方式 | 原因 |
|------|---------|------|
| bge-large-zh-v1.5 | sentence-transformers | 向量编码模型，非自回归生成 |
| Apollo-0.5B | transformers pipeline (text-classification) | 分类模型，单次前向推理 |
...
| Apollo-0.5B | transformers pipeline | PASS | ~1.0 GB | 分类 label+score 正常 |
| nlp_raner | transformers pipeline | PASS | ~0.4 GB | NER 实体提取正常 |
| MedGemma-1.5-4B-IT | SGLang Runtime | FAIL | — | 输出全零 token，架构不兼容 |
| MedPsy-1.7B | SGLang Runtime | PASS | ~4.1 GB | Qwen3ForCausalLM 一等公民 |
...
| MedPsy-1.7B (SGLang :30001) | SGLang Runtime | — |
| bge-large-zh | sentence-transformers | ~1.4 GB |
| Apollo-0.5B | transformers pipeline | ~1.0 GB |
...
| MedPsy-1.7B | 健康评估 514 字 | 5.00s | OK |
...
| MedPsy-1.7B | 健康评估 816 字 / 512 token | 15.50s |
...
| MedPsy-1.7B | 健康评估 768 字 / 512 token | 15.49s |
...
| 组件 | 模型 | 加载方式 | 端口 | 显存 |
|------|------|---------|------|------|
| 主推理 | Qwen3-4B-AWQ | SGLang Runtime | 30000 | ~9.96 GB |
| 健康评估 | MedPsy-1.7B | SGLang Runtime | 30001 | ~4.1 GB |
| 向量编码 | bge-large-zh-v1.5 | sentence-transformers | — | ~1.4 GB |
| 意图分类 | Apollo-0.5B | transformers pipeline | — | ~1.0 GB |
...
| 健康评估 (512 token 输出) | MedPsy-1.7B | ~15s | FP16，小模型推理快 |
| 向量编码 | bge-large-zh | ~0.3s | 单次编码 |
| 意图分类 | Apollo-0.5B | ~0.2s | 单次分类 |
