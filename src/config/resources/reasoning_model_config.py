...
@dataclass
class ReasoningModelConfig(BaseResourceConfig):
    """
    通用推理模型资源配置类
    ...
    属性：
        config_id: 配置ID
        resource_type: 资源类型
        base_url: SGLang HTTP服务地址（运行期由yaml覆盖）
        ...
        default_repetition_penalty: 默认重复惩罚（占位默认值，运行期由yaml覆盖）
    """

    config_id: str = "reasoning_model_config"
    resource_type: str = "reasoning_model"
    base_url: str = ""
    model_name: str = ""
...
resource_config = ReasoningModelConfig()

resource_type = "reasoning_model"
